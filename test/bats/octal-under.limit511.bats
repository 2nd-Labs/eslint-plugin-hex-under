...
run_fixture() {
  ...
  run npx eslint --fix "$TMP_DIR/input.js" --config test/bats/bats-eslint.config.js
  if [ "$status" -ne 0 ]; then
...
@test "fixture: example.octal.limit511.js" {
  run_fixture "$BATS_TEST_DIRNAME/fixture/limit/example.octal.limit511.js"
}
